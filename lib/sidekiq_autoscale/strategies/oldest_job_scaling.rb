  ...
  module Strategies
    class OldestJobScaling < BaseScaling
      ...
      def workload_too_high?
        ...
        SidekiqAutoscale.logger.debug("#{LOG_TAG} Current average delay: #{sidekiq_interface.latency}, max allowed: #{@scale_up_threshold}")
        too_high
      end
      ...
      def workload_too_low?
        ...
        SidekiqAutoscale.logger.debug("#{LOG_TAG} Current average delay: #{sidekiq_interface.latency}, min allowed: #{@scale_down_threshold}")
        too_low
      end
    end
  end
