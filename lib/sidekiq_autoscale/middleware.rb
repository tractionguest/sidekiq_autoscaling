class SidekiqAutoscale::Middleware
  STRATEGIES = SidekiqAutoscaling::BaseScaling.subclasses.freeze
  ...
  def initialize(strategy:,
                 ...
                 adapter: ::SidekiqAutoscaling::HerokuAdapter.new,
                 sidekiq_interface: ::SidekiqAutoscaling::SidekiqInterface,
...
[SIDEKIQ_SCALE] Provider adapter: #{@adapter.class.name}
  ...
  def self.strategy_picker(strat)
    strat_klass_name = SidekiqAutoscaling::STRATEGIES.map(&:to_s).find { |i| i.end_with?("#{strat.to_s.camelize}Scaling") }
    raise StandardError("#{LOG_TAG} Unknown scaling strategy: [#{strat.to_s.camelize}Scaling]") if strat_klass_name.nil?
    
    return strat_klass_name.constantize
  end
