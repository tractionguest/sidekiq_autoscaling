...
RSpec.describe SidekiqAutoscale::Middleware, type: :model do
  ...
  let(:on_error_block) { Proc.new { |_e| @error_block_fired = true} }
  ...
  context "when scaling strategy raises an error on logger" do
    before do
      TestStrategy.instance.log_job_proc = Proc.new { |_| raise StandardError.new }
    end
  ...
  context "when scaling strategy raises an error on workload change" do
    before do
      TestStrategy.instance.workload_change_proc = Proc.new { |_| raise StandardError.new }
    end
  ...
  context "when scaling strategy requires scaling up" do
    ...
    before do
      ...
      TestStrategy.instance.workload_change_proc = Proc.new { |_| true }
      TestStrategy.instance.scaling_direction_proc = Proc.new { |_| 1 }
      TestAdapter.instance.set_worker_count_proc = Proc.new { |n| 
        byebug
        @worker_change = n }
      TestAdapter.instance.get_worker_count_proc = Proc.new { 1 }
    end
