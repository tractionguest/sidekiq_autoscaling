...
RSpec.describe SidekiqAutoscale::Strategies::DynamicLatencyScaling, type: :model do
  ...
  context "when scaling up is needed" do
    let(:current_oldest_job) { 2.5 }

    it { expect(strategy.workload_change_needed?(nil)).to eq true }
    it { expect(strategy.scaling_direction(nil)).to eq 1 }

    context "when lots of scaling up is needed" do
      let(:current_oldest_job) { 11.0 }

      it { expect(strategy.workload_change_needed?(nil)).to eq true }
      it { expect(strategy.scaling_direction(nil)).to eq 10 }
    end

    context "when double the usual scaling up is needed" do
      let(:current_oldest_job) { 3.0 }

      it { expect(strategy.workload_change_needed?(nil)).to eq true }
      it { expect(strategy.scaling_direction(nil)).to eq 2 }
    end
  end
